...
import { LayoutDashboard, Package, Layers, LogOut, User, Briefcase, FileText, ShoppingCart, X, Settings, DollarSign, BarChart2, Plus, Truck, Download, Bell, Info, AlertTriangle, Trash2, ChevronRight } from 'lucide-react';
...
export default function MainLayout() {
    ...
    const SidebarContent = () => (
        <>
            <div className={`p-8 flex items-center justify-center border-b border-white/10 mx-4 transition-all duration-300 ${isCollapsed ? 'px-2 py-6' : ''}`}>
                <h1 className={`font-black tracking-tighter text-white filter drop-shadow-[0_0_10px_rgba(255,255,255,0.3)] text-center break-words leading-tight transition-all duration-300 ${isCollapsed ? 'text-2xl' : 'text-xl'}`}>
                    {isCollapsed ? companyName.charAt(0) : companyName.toUpperCase()}
                </h1>
            </div>
                ...
                {navItems.map((item) => (
                    <NavLink
                        ...
                        className={({ isActive }) =>
                            ...
                            } ${isCollapsed ? 'w-11 h-11 justify-center rounded-xl mx-auto' : 'px-4 py-3 gap-3 rounded-xl'}`
                        }
                    >
                        {({ isActive }) => (
                            <>
                                {isActive && <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500 to-violet-500 rounded-xl -z-10 opacity-100"></div>}
                                <item.icon size={isCollapsed ? 20 : 20} className={`shrink-0 transition-transform duration-300 ${isActive ? 'scale-110' : 'group-hover:scale-110'}`} />
                                <span className={`tracking-wide whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
                                    {item.label}
                                </span>
                            </>
                        )}
                            ...
                            {isNotifOpen && (
                                <>
                                    ...
                                    <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-slate-100 p-2 animate-scale-in origin-top-right z-50 max-h-[500px] overflow-hidden flex flex-col">
                                        <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/30 rounded-t-xl">
                                            ...
                                            {unreadCount > 0 && (
                                                <button
                                                    ...
                                                    className="text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-700 tracking-wider"
                                                >
                                                    Marcar todo leido
                                                </button>
                                            )}
                                            ...
                                            {notifications.length === 0 ? (
                                                ...
                                                <div className="space-y-1">
                                                    {notifications.map(n => (
                                                        <div
                                                            key={n.id}
                                                            className={`p-3 rounded-xl transition-all cursor-pointer group flex items-start gap-3 ${n.isRead ? 'opacity-60 bg-transparent' : 'bg-indigo-50/30 hover:bg-white'}`}
                                                            onClick={() => !n.isRead && markAsRead(n.id)}
                                                        >
                                                            <div className={`mt-1 p-2 rounded-lg ${n.type === 'Warning' ? 'bg-amber-100 text-amber-600' :
                                                            ...
                                                            <div className="flex-1">
                                                                ...
                                                                {n.link && (
                                                                    <NavLink
                                                                        ...
                                                                        className="inline-block mt-2 text-[10px] font-black text-indigo-600 hover:underline uppercase tracking-widest"
                                                                    >
                                                                        Ver detalle &rarr;
                                                                    </NavLink>
                                                                )}
