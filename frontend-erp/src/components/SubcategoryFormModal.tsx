...
const SubcategoryFormModal: React.FC<SubcategoryFormModalProps> = ({ subcategory, categoryId, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/10 backdrop-blur-[2px] animate-fade-in">
            <div className="bg-white p-8 rounded-3xl w-full max-w-md shadow-[0_20px_50px_rgba(0,0,0,0.1)] animate-scale-in border border-slate-100">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-2xl font-bold text-slate-800 tracking-tight">{isEdit ? 'Editar Subcategoría' : 'Nueva Subcategoría'}</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 transition-colors hover:bg-slate-100/50">
                        ...
                        />
                        <label htmlFor="subcatActive" className="text-sm font-bold text-slate-600 cursor-pointer select-none">Subcategoría Activa</label>
                    </div>
